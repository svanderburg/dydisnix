...
static NixXML_bool assign_global_ids_to_distributed_services(GHashTable *id_assignments_table, GHashTable *last_assignments_table, gchar *resource_name, IdResourceType *type, GHashTable *services_table, GHashTable *distribution_table, gchar *service_property)
{
...
static NixXML_bool assign_machine_level_ids_to_distributed_services(GHashTable *id_assignments_table, GHashTable *last_assignments_per_target_table, gchar *resource_name, IdResourceType *type, GHashTable *services_table, gchar *service_property, GHashTable *target_to_service_table)
{
    ...
    while(NixXML_g_hash_table_ordered_iter_next(&iter, &target_name, &value))
    {
        ...
        GHashTable *last_assignments_table = retrieve_or_add_empty_last_assignments_table_for_target(last_assignments_per_target_table, target_name);

        if(!create_id_assignments_for_services(id_assignments_table, last_assignments_table, service_names, resource_name, type, services_table, service_property))
        {
            result = FALSE;
            break;
        }
...
NixXML_bool assign_ids_to_distributed_services(GHashTable *id_resources_table, IdsConfig *ids_config, GHashTable *services_table, GHashTable *distribution_table, GHashTable *target_to_service_table, gchar *service_property)
{
    ...
    while(g_hash_table_iter_next(&iter, &key, &value))
    {
        ...
        {
            if(!assign_global_ids_to_distributed_services(id_assignments_table, ids_config->last_assignments_table, resource_name, type, services_table, distribution_table, service_property))
                return FALSE;
        }
        else if(xmlStrcmp(type->scope, (xmlChar*) "machine") == 0)
        {
            if(!assign_machine_level_ids_to_distributed_services(id_assignments_table, ids_config->last_assignments_per_target_table, resource_name, type, services_table, service_property, target_to_service_table))
                return FALSE;
        }
        else
        {
